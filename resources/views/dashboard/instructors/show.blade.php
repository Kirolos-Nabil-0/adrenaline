...
@section('content-dashboard')
    ...
    <div class="d-flex mt-5">
        <h4 class="content-title mb-0 my-auto">Manage <span class="text-muted mt-1 tx-13 mr-2 mb-0"> / Instructors / {{$instructor->firstname}}</span></h4>
    </div>
    <div class="page-details mt-3">
        ...
        <div class="card shadow">
            <div class="card-body">
                <h4 class="text-primary">Instructor Details</h4>
                <div class="row mb-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <p>{{$instructor->owned_courses->count()}}</p>
                    </div>
                    @if ($instructor->center)
                        <div class="col-md-6">
                            <strong>Associated Center:</strong>
                            <p><a href="{{route("centers.show", $instructor->center)}}">{{$instructor->center->firstname}}</a></p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                @if($instructor->packages->count() > 0)
                    <div class="table-responsive">
                        <table class="table text-md-nowrap table-bordered" id="collages">
                            ...
                            <tbody>
                                ...
                                @foreach ($instructor->packages as $package)
                                    @php
                                        $i++;
                                    @endphp
                                    <tr>
                                        ...
                                        <td>
                                            @if ($instructor->currentDeactivePackage() && $instructor->currentDeactivePackage()->id == $package->id)
                                                <a class="modal-effect btn btn-sm btn-success d-flex gap-1" data-effect="effect-scale"
                                                data-id="{{ $instructor->id }}" data-name="{{ $package->name }}"
                                                data-toggle="modal" href="#modaldemo9" title="تفعيل">Activate <i
                                                    class="las la-star"></i></a>
                                            @endif
                                            @if ($instructor->currentPackage() && $instructor->currentPackage()->id == $package->id)
                ...
                @else
                    <p>No packages associated with this instructor.</p>
                @endif
            </div>
        </div>
    </div>
...
@section('page_js')
    ...
    <script>
        $('#modaldemo9').on('show.bs.modal', function(event) {
            ...
            var actionUrl = '{{ route("instructors.activate-package", ":id") }}'; 
        ...
        $('#modaldemo10').on('show.bs.modal', function(event) {
            ...
            var actionUrl = '{{ route("instructors.deactivate-package", ":id") }}'; 
            actionUrl = actionUrl.replace(':id', id);
